...
from semanticmodels2.models.word2vec import W2Vec
import torch
...
class evaluation:
    def __init__(self, average_over_models = False):
        ...
        self.export_path = Path('/Users/jingfengzhang/FirstYearProject/SemanticModels2/results/{}'.format(model_type))
        self.export_path.mkdir(exist_ok=True)
        # config.Dirs.runs if loading runs locally or None if loading data from ludwig

        project_name = "SemanticModels2"
        # model_type = 'word2vec'
        ...
        self.category_colnames = {'0':'Period', '1':'Present A', '2': 'Omitted A', '3': 'Legal As', '4': 'Illegal As', '5': 'Present B' ,'6':'Omitted B', '7':'Legal Bs', '8':'Illegal Bs', '9': 'y'}
        self.dsm_list = []
        i = 0
        self.current_dir = ''
        for param_path, label in gen_param_paths(project_name,
            ...
            for i in range(len(corpus.numeric_token_sequence_sentence_list[0])):
                current_sequence_test_category_array = np.array(corpus.sequence_test_category_list[i])
                category_dict = {list(self.dsm_list[0].id2token.values())[j]: current_sequence_test_category_array[j] for j in range(self.dsm_list[0].vocab_size)}
                ...
                print(f'current sentence: {[self.dsm_list[0].id2token[token] for token in corpus.numeric_token_sequence_sentence_list[0][i]]}')
                for value, keys in labeled_dict.items():
                    print(f"Category: {value}, Tokens: {keys}")

                
            dsm_info = [self.dsm_list[0].params.num_epochs, self.dsm_list[0].params.learning_rate, self.dsm_list[0].params.embed_size, 
                        self.dsm_list[0].params.embed_init_range ,self.dsm_list[0].params.momentum, self.dsm_list[0].params.round]
            self.current_dir = str(self.export_path)+'/{}_{}_{}_{}_{}_{}'.format(dsm_info[0], dsm_info[1], dsm_info[2],dsm_info[3],dsm_info[4],dsm_info[5])
            os.makedirs(self.current_dir, exist_ok=True)
            
            dsm_weight_similarity_matrix_list = list(map(lambda embed_matrix: create_similarity_matrix(embed_matrix, 'corrcoef'), dsm_weight_list))
            self.dsms_weight_similarity_matrix_list.append(dsm_weight_similarity_matrix_list)
            dsm_hidden_similarity_matrix_list = list(map(lambda embed_matrix: create_similarity_matrix(embed_matrix, 'corrcoef'), dsm_hidden_activation_list))
            ...
            self.combine_and_export_data(dsm_weight_similarity_matrix_list, self.checkpoint_list, self.current_dir+'/combined_weights_from_different_checkpoints.csv')
            self.combine_and_export_data(dsm_hidden_similarity_matrix_list, self.checkpoint_list, self.current_dir+'/combined_hidden_from_different_checkpoints.csv')
            self.combine_and_export_data(dsm_output_activation_list, self.checkpoint_list, self.current_dir+'/combined_outputs_from_different_checkpoints.csv')
            ...
            plot_similarity_matrices(dsm_weight_similarity_matrix_list, 'weight',self.checkpoint_list ,self.words, self.groups, self.current_dir)
            plot_similarity_matrices(dsm_hidden_similarity_matrix_list, 'hidden',self.checkpoint_list ,self.words, self.groups, self.current_dir)
            ...
            for i in range(len(self.checkpoint_list)):
                cohyponymtask_using_weights = CohyponymTask(corpus.paradigmatic_word_category_dict, dsm_weight_similarity_matrix_list[i] , dsm.vocab_dict)
                cohyponymtask_using_hidden = CohyponymTask(corpus.paradigmatic_word_category_dict, dsm_hidden_similarity_matrix_list[i], dsm.vocab_dict)
                ...
                mean_acivtation_weights, sum_acivtation_weights, guess_accuracies_weights = evaluate_model(self.dsm_list[i], model_type, self.checkpoint_list[i], corpus.numeric_token_sequence_list, 
                                                                                                        corpus.sequence_test_category_list,dsm_weight_similarity_matrix_list[i],
                                                                                                        cohyponymtask_using_weights)
                mean_acivtation_hidden, sum_acivtation_hidden, guess_accuracies_hidden = evaluate_model(self.dsm_list[i], model_type, self.checkpoint_list[i], corpus.numeric_token_sequence_list, 
                                                                                                            corpus.sequence_test_category_list,dsm_hidden_similarity_matrix_list[i],
                                                                                                            cohyponymtask_using_hidden)
                mean_acivtation_outputs, sum_acivtation_outputs, guess_accuracies_outputs= evaluate_model(self.dsm_list[i], model_type, self.checkpoint_list[i], corpus.numeric_token_sequence_list, corpus.sequence_test_category_list)
                for j in range(3):
                ...
                if i == len(self.checkpoint_list) - 1:
                    cohyponymtask_using_weights.guess_accuracy_best_df.to_csv(self.current_dir+'/weight_guess_accuracy.csv')
                    cohyponymtask_using_hidden.guess_accuracy_best_df.to_csv(self.current_dir+'/hidden_guess_accuracy.csv')
            temp_dsms_cohyponymtask_using_weights_df = self.merge_data_from_checkpoints(dsm_cohyponymtask_using_weights_list, self.dsms_cohyponymtask_using_weights_df_list)
            temp_dsms_cohyponymtask_using_hidden_df = self.merge_data_from_checkpoints(dsm_cohyponymtask_using_hidden_activation_list, self.dsms_cohyponymtask_using_hidden_df_list)
            temp_dsms_syntagmatictask_using_sum_outputs_df = self.merge_data_from_checkpoints(dsm_syntagmatictask_using_sum_outputs_list,self.dsms_syntagmatictask_using_sum_outputs_df_list)
            temp_dsms_syntagmatictask_using_mean_outputs_df = self.merge_data_from_checkpoints(dsm_syntagmatictask_using_mean_outputs_list,self.dsms_syntagmatictask_using_mean_outputs_df_list)
            ...
            plot_cohyponym_score(temp_dsms_cohyponymtask_using_weights_df, temp_dsms_cohyponymtask_using_hidden_df, self.current_dir+'/cohyponym_plot.png')
    ...
    def combine_and_export_data(self, data_list, keys, dir):
        vocab_list = list(self.dsm_list[0].vocab_dict.keys())
    ...
    def average_results(self):
        ...
        cohyponymtask_using_weights_df = self.average_over_multiple_models(self.dsms_cohyponymtask_using_weights_df_list)
        cohyponymtask_using_hidden_df = self.average_over_multiple_models(self.dsms_cohyponymtask_using_hidden_df_list)
        syntagmatictask_using_sum_outputs_df = self.average_over_multiple_models(self.dsms_syntagmatictask_using_sum_outputs_df_list)
        syntagmatictask_using_mean_outputs_df = self.average_over_multiple_models(self.dsms_syntagmatictask_using_mean_outputs_df_list)
        ...
        plot_cohyponym_score(cohyponymtask_using_weights_df, cohyponymtask_using_hidden_df, average_dir+'/cohyponym_average_plot.png')
        plot_similarity_matrices(self.average_lists_of_matrices(self.dsms_weight_similarity_matrix_list), 'weight',self.checkpoint_list ,self.words, self.groups, average_dir)
        plot_similarity_matrices(self.average_lists_of_matrices(self.dsms_hidden_similarity_matrix_list), 'hidden',self.checkpoint_list ,self.words, self.groups, average_dir)
eval = evaluation()
